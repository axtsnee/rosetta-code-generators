...
case class ScalaEnumerationGenerator(
    analysis: RootElementAnalyzer
) extends AbstractScalaGenerator(analysis.enums) {
  ...
  override val derivePackageName: RosettaEnumeration => String =
    ScalaEnumerationGenerator.derivePackageName
...
object ScalaEnumerationGenerator {
  def derivePackageName(e: RosettaEnumeration): String =
    s"${AbstractScalaGenerator.deriveParentPackage(e)}.enums"
}
